...
|---|---|---|---|---|
| **`@nccirtu/tablefy`** | [`packages/tablefy`](./packages/tablefy) | TypeScript/React | `npm install @nccirtu/tablefy` | DataTable, ~17 Spalten, Forms, Inertia-Integration (Frontend) |
| **`nccirtu/tablefy-php`** | [`packages/tablefy-php`](./packages/tablefy-php) | PHP/Laravel | `composer require nccirtu/tablefy-php` | `make:tablefy-resource`-Generator, CRUD-Controller, `tablefy()`-Query-Macro (Backend) |
...
npm run build        # = npm run build -w @nccirtu/tablefy
npm test             # = npm run test  -w @nccirtu/tablefy
# oder direkt:

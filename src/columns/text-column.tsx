// lib/table/columns/text-column.tsx
...
export class TextColumn<TData> extends BaseColumn<TData, FormattedColumnConfig<TData>> {
    ...
    uppercase(): this {
        ...
        this.config.formatter = (value, row): React.ReactNode => {
            ...
            return typeof result === 'string' ? result.toUpperCase() : (result as React.ReactNode);
        };
        return this;
    }
    ...
    lowercase(): this {
        ...
        this.config.formatter = (value, row): React.ReactNode => {
            const result = existingFormatter ? existingFormatter(value, row) : value;
            return typeof result === 'string' ? result.toLowerCase() : (result as React.ReactNode);
        };
        return this;
    }
        ...
        return {
            ...
            cell: ({ row, getValue }) => {
                ...
                if (value === null || value === undefined || value === '') {
                    return (
                        <span className={cn('text-muted-foreground', this.getAlignmentClass(), this.config.cellClassName)}>{placeholder || '—'}</span>
                    );
                }
                ...
                return <span className={cn(this.getAlignmentClass(), this.config.cellClassName)}>{displayValue}</span>;
            },
        };
    }
}

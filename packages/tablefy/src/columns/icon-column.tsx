...
export class IconColumn<TData> extends BaseColumn<
  TData,
  IconColumnConfig<TData>
> {
  ...
  build(): ColumnDef<TData, unknown> {
    ...
    return {
      ...
      cell: ({ getValue }) => {
        ...
        const IconElement =
          typeof icon === "function" ? (icon as LucideIcon) : null;
        const renderedIcon = IconElement ? (
          <IconElement className="h-full w-full" />
        ) : (
          (icon as ReactNode)
        );
        ...
        const iconElement = (
          ...
          >
            {withBackground ? (
              ...
              >
                <div className={cn(iconInBgSizeClasses[size || "md"], color)}>
                  {renderedIcon}
                </div>
              </div>
            ) : (
              <div className={cn(sizeClasses[size || "md"], color)}>
                {renderedIcon}
              </div>
            )}

            {showLabel && (
              <span className={cn("text-sm", color)}>{stateLabel}</span>
            )}
          </div>
        );

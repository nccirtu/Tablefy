...
export class DateColumn<TData> extends BaseColumn<
  TData,
...
> {
  ...
  private getRelativeTime(date: Date): string {
    ...
    for (const interval of intervals) {
      ...
      if (count >= 1) {
        const plural =
          count > 1 ? (interval.label === "Monat" ? "e" : "en") : "";
  ...
  build(): ColumnDef<TData, unknown> {
    ...
    return {
      ...
      meta: { visibilityLabel: label || String(accessor) },
      header: ({ column }) => {
      ...
      cell: ({ getValue }) => {
        ...
        if (isNaN(date.getTime())) {
          return (
            <span className="text-muted-foreground">Ungültiges Datum</span>
          );
        }

        const formatted = this.formatDate(
          date,
          format || "short",
          locale || "de-DE",
        );

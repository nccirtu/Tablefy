...
import { DataTableConfig, BulkAction } from "../types/table";
import { HeaderAction } from "../types/actions";
import { EmptyStateConfig } from "../types/empty-state";
import { FilterConfig, SearchConfig } from "../types/filters";
import React from "react";

type FilterInput = FilterConfig | { build(): FilterConfig };
...
export class TableSchema<TData> {
  ...
  /** Actions that run on the selected rows (shown in a bar when rows are selected). */
  bulkActions(actions: BulkAction<TData>[]): this {
    this.config.enableRowSelection = true;
  ...
  // Filters (accepts fluent builders or plain FilterConfig objects)
  filters(...filters: FilterInput[]): this {
    this.config.filters = filters.map((f) =>
      "build" in f ? f.build() : f,
    );
    return this;
  }
